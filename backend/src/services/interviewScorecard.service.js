...
export const getScorecard = async (company, roundId, userId) => {
  ...
  const isOwner = comp?.owner?.toString() === userId.toString();

  const jobObj = await mongoose.model('Job').findById(round.job);
  const isHiringManager = jobObj?.hiringManager?.toString() === userId.toString();

  const isInterviewer = round.interviewers.some((id) => id.toString() === userId.toString());

  if (!isOwner && !isHiringManager && !isInterviewer) {
    throw new AppError('Unauthorized access to interview scorecard', 403);
  }

  const [schedule, feedback] = await Promise.all([
    InterviewSchedule.findOne({ round: roundId, company }).lean(),
    isInterviewer
      ? InterviewFeedback.findOne({ round: roundId, company, interviewer: userId })
          .select('+privateNotes')
          .lean()
      : null,
  ]);
...
export const listScorecards = async (company, interviewer, overdue = false) => {
  ...
  const scheduleByRound = new Map(schedules.map((item) => [String(item.round), item]));
  const feedbackByRound = new Map(feedback.map((item) => [String(item.round), item]));
  return rounds.flatMap((round) => {
    const ownFeedback = feedbackByRound.get(String(round._id));
    if (ownFeedback?.submitted) return [];
    const dto = serializeScorecard(round, scheduleByRound.get(String(round._id)), ownFeedback);
