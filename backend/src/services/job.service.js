...
export const createJob = async (company, userId, input) => {
  ...
  return Job.create({ ...input, slug, company: company.id, createdBy: userId, status: 'draft' });
};
...
export const updateJob = async (companyId, jobId, input) => {
  ...
  Object.entries(input).forEach(([key, value]) => job.set(key, value)); ensureValidRanges(job); await job.save(); return job;
};
export const archiveJob = async (companyId, jobId) => {
  const job = await ownedJob(companyId, jobId);
...
export const submitJob = async (company, jobId) => {
  ensureCompanyOperational(company); const job = await ownedJob(company.id, jobId); ensurePublishable(job); transitionJob(job, 'pending-review'); await job.save(); const recipients = await platformAdmins(); if (recipients.length) await publishOptionalDomainEvent({ type: DOMAIN_EVENTS.JOB_SUBMITTED, company: String(company.id), recipientIds: recipients.map(String), payload: { companyId: String(company.id), companyName: company.name, jobId: String(job.id), jobTitle: job.title, actionUrl: `/admin/jobs/${job.id}` }, deduplicationKey: `job.submitted:${job.id}:${job.updatedAt.toISOString()}` }); return job;
};
export const recruiterPublishJob = async () => { throw new AppError('Jobs are published only through admin approval', 403); };
export const pauseJob = async (companyId, jobId) => { const job = await ownedJob(companyId, jobId); transitionJob(job, 'paused'); await job.save(); await publishOptionalDomainEvent({ type: DOMAIN_EVENTS.JOB_PAUSED, company: String(companyId), recipientIds: [String(job.createdBy)], payload: { companyId: String(companyId), jobId: String(job.id), jobTitle: job.title }, deduplicationKey: `job.paused:${job.id}:${job.updatedAt.toISOString()}` }); return job; };
export const resumeJob = async (company, jobId) => { ensureCompanyOperational(company); const job = await ownedJob(company.id, jobId); ensurePublishable(job); transitionJob(job, 'published'); await job.save(); return job; };
export const closeJob = async (companyId, jobId) => { const job = await ownedJob(companyId, jobId); transitionJob(job, 'closed'); await job.save(); await publishOptionalDomainEvent({ type: DOMAIN_EVENTS.JOB_CLOSED, company: String(companyId), recipientIds: [String(job.createdBy)], payload: { companyId: String(companyId), jobId: String(job.id), jobTitle: job.title }, deduplicationKey: `job.closed:${job.id}:${job.updatedAt.toISOString()}` }); return job; };
