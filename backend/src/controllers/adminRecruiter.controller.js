import { approveRecruiter, listPendingRecruiters, rejectRecruiter, suspendRecruiter, restoreRecruiter } from '../services/adminRecruiter.service.js';
const action = (service, message) => async (request, response, next) => { try { const profile = await service(request.params.recruiterId, request.user.id, request.body.reason, request.ip, request.headers['user-agent']); return response.json({ success: true, message, data: { profile } }); } catch (error) { return next(error); } };
...
export const restoreRecruiterAccount = action(restoreRecruiter, 'Recruiter restored successfully');
